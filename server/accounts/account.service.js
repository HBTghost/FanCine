...
async function authenticate({ email, password, ipAddress }) {
  ...
  const _refreshToken = generateRefreshToken(account, ipAddress);
...
async function refreshToken({ token, ipAddress }) {
  const _refreshToken = await getRefreshToken(token);
  const { account } = _refreshToken;

  // replace old refresh token with a new one and save
  const newRefreshToken = generateRefreshToken(account, ipAddress);
  _refreshToken.revoked = Date.now();
  _refreshToken.revokedByIp = ipAddress;
  _refreshToken.replacedByToken = newRefreshToken.token;
  await _refreshToken.save();
...
async function register(params, origin) {
  ...
  // send email
  const res = await sendVerificationEmail(account, origin);
  return res;
}
