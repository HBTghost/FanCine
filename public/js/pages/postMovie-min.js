...
(function ($) {
  ...
  TagsInput.prototype = {
    ...
    add(item, dontPushVal, options) {
      ...
      const existing = $.grep(self.itemsArray, (item) => self.options.itemValue(item) === itemValue)[0];
      if (existing && !self.options.allowDuplicates) {
        // Invoke onTagExists
        if (self.options.onTagExists) {
          const $existingTag = $('.badge', self.$container).filter(function () { return $(this).data('item') === existing; });
    ...
    remove(item, dontPushVal, options) {
      ...
      if (self.objectItems) {
        if (typeof item === 'object') item = $.grep(self.itemsArray, (other) => self.options.itemValue(other) == self.options.itemValue(item));
    ...
    build(options) {
      ...
      if (self.options.typeaheadjs) {
        ...
        $.fn.typeahead.apply(self.$input, typeaheadjs).on('typeahead:selected', $.proxy((obj, datum, name) => {
          let index = 0;
