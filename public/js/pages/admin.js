...
(function ($) {
  ...
  function TagsInput(element, options) {
    ...
    this.$input = $("<input type=\"".concat(this.type, "\" name=\"").concat(this.name, "\" placeholder=\"").concat(this.placeholderText, "\"/>")).appendTo(this.$container);
  ...
  TagsInput.prototype = {
    ...
    add: function add(item, dontPushVal, options) {
      ...
      var itemTitle = self.options.itemTitle(item); // Ignore items allready added

      var existing = $.grep(self.itemsArray, function (item2) {
        return self.options.itemValue(item2) === itemValue;
      })[0];
      ...
      var $tag = $("<span class=\"".concat(htmlEncode(tagClass)).concat(itemTitle !== null ? "\" title=\"".concat(itemTitle) : '', "\">").concat(htmlEncode(itemText), "<span data-role=\"remove\"></span></span>"));
      $tag.data('item', item);
      self.findInputWrapper().before($tag); // Check to see if the tag exists in its raw or uri-encoded form

      var optionExists = $("option[value=\"".concat(encodeURIComponent(itemValue).replace(/"/g, '\\"'), "\"]"), self.$element).length || $("option[value=\"".concat(htmlEncode(itemValue).replace(/"/g, '\\"'), "\"]"), self.$element).length; // add <option /> if item represents a value not present in one of the <select />'s options

      if (self.isSelect && !optionExists) {
        var $option = $("<option selected>".concat(htmlEncode(itemText), "</option>"));
    ...
    refresh: function refresh() {
      ...
      $('.badge', self.$container).each(function () {
        ...
        var itemValue = self.options.itemValue(item);
        var itemText = self.options.itemText(item);
        var tagClass = self.options.tagClass(item); // Update tag's class and inner text

        $tag.attr('class', null);
        $tag.addClass("badge ".concat(htmlEncode(tagClass)));
      ...
      if (self.options.typeahead) {
        ...
        self.$input.typeahead($.extend({}, typeahead, {
          ...
          highlighter: function highlighter(text) {
            var regex = new RegExp("(".concat(this.query, ")"), 'gi');
