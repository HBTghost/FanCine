...
gulp.task('serve', gulp.series('nodemon', (done) => {
  ...
  gulp.watch(['**/*.js', '!node_modules/**', '!public/js/**'], gulp.series('lint-js-fix'));
  done();
}));
...
gulp.task('lint-js-fix', () => gulp.src(['./**/*.js', '!node_modules/**', '!public/js/**'])
  .pipe(eslint(({ useEslintrc: true, fix: true })))
  .pipe(eslint.format())
  .pipe(eslint.failAfterError())
  .pipe(eslintIfFixed('./')));

gulp.task('compileJS', () => gulp.src('public/njs/**/*.js')
  .pipe(babel())
  .pipe(minify())
  .pipe(gulp.dest('public/js')));

gulp.task('lint-js', () => gulp.src(['./**/*.js', '!node_modules/**', '!public/js/**'])
  .pipe(eslint(({ useEslintrc: true })))
  .pipe(eslint.format())
  .pipe(eslint.failAfterError()));

gulp.task('lint-sass', () => gulp.src('public/scss/**/*.scss')
